...
export const templates = {
  ...
  date: (date = new Date()) =>
    `${date.toISOString().replace(/-/gi, '.').replace('T', ' ').slice(0, 19)}`,
  torrent: {
    view(torrentId, torrent, url) {
      let messageText = `<a href='https://${
        env.HOST
      }/view/${torrentId}'>${decode(torrent.name)
        .replace(/</gi, '&lt;')
        .replace(/>/gi, '&gt;')
        .replace(/&/gi, '&amp;')}</a>\n`
      if (torrent.entry) {
      ...
      messageText += `<b>Submitter:</b> ${
        typeof torrent.submitter === 'string'
          ? torrent.submitter
          : `<a href='${torrent.submitter.link}'>${torrent.submitter.name}</a>`
      }\n`
      messageText += `<b>Info:</b> ${torrent.info}\n`
      messageText += `${torrent.file_size} · ${templates.date(
    ...
    filesView(torrent, offset, url) {
      let messageText = `<a href='https://${env.HOST}/view/${
        torrent.id
      }'>${decode(torrent.name)
        .replace(/</gi, '&lt;')
        .replace(/>/gi, '&gt;')
    ...
    inlineQuery(torrent) {
      ...
      messageText += `#c${
        Array.isArray(torrent.category)
          ? torrent.category[1].code
          : torrent.category.code
      } ${
        Array.isArray(torrent.category)
          ? torrent.category
              .map(
                (el) =>
                  `<a href='https://${env.HOST}/?c=${el.code}'>${el.title}</a>`
              )
              .join(' - ')
          : `<a href='https://${env.HOST}/?c=${torrent.category.code}'>${torrent.category.label}</a>`
      }`
      return messageText
    }
  },
