...
export const templates = {
  ...
  torrent: {
    ...
    inlineQuery (torrent) {
      ...
      messageText += `${torrent.file_size} · ${templates.date(new Date(torrent.timestamp))} · ⬆️ ${torrent.stats.seeders} · ⬇️ ${torrent.stats.leechers} · ☑️ ${torrent.stats.downloaded}\n`
      messageText += `#c${Array.isArray(torrent.category) ? torrent.category[1].code : torrent.category.code} ${Array.isArray(torrent.category) ? torrent.category.map(el => `<a href='https://${env.HOST}/?c=${el.code}'>${el.title}</a>`).join(' - ') : `<a href='https://${env.HOST}/?c=${torrent.category.code}'>${torrent.category.label}</a>`}`
      return messageText
    }
  },
