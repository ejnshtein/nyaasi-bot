...
const templates = {
  ...
  torrent: {
    view (torrentId, torrent, url) {
      let messageText = `<a href="https://nyaa.si/view/${torrentId}">${decode(torrent.title)}</a>\n`
      if (torrent.entry) {
    ...
    inlineQuery (torrent) {
      let messageText = `\n<a href="https://nyaa.si/view/${torrent.id}">${decode(torrent.title)}</a>\n`
      messageText += `${torrent.fileSize} · ${templates.date(new Date(Number.parseInt(torrent.timestamp) * 1000))} · ⬆️ ${torrent.seeders} · ⬇️ ${torrent.leechers} · ☑️ ${torrent.completed}\n`
      messageText += `#c${torrent.category[1].code} ${torrent.category.map(el => `<a href="https://nyaa.si/?c=${el.code}">${el.title}</a>`).join(' - ')}`
      return messageText
    }
  }
}
